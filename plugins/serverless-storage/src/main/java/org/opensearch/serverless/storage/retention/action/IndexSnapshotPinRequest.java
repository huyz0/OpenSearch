...
/** Names an index by its name (not UUID -- resolved from cluster state) and a {@code snapshotId} for {@link IndexSnapshotPinAction}. */
public class IndexSnapshotPinRequest extends ActionRequest {
     ...
     * Creates a request.
     *
     * @param indexName name of the index to pin, every shard, under {@code snapshotId}.
     * @param snapshotId the snapshot name to pin every shard's current manifest generation under.
     */
    public IndexSnapshotPinRequest(String indexName, String snapshotId) {
     ...
     *
     * @param in stream positioned at a previously-{@link #writeTo}-written {@link IndexSnapshotPinRequest}.
     */
    public IndexSnapshotPinRequest(StreamInput in) throws IOException {
        super(in);
    ...
    /** Name of the index to pin every shard of. */
    public String indexName() {
        return indexName;
    }

    /** The snapshot name to pin every shard's current manifest generation under. */
