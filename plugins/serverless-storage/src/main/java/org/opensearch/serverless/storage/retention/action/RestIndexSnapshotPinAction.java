...
/**
 * {@code POST /_plugins/_serverless/storage/index/{index}/_snapshot_pin} with a body of the shape
 * {@code {"snapshot_id": "..."}} -- the REST surface for {@link IndexSnapshotPinAction}
 * (rfc-serverless-opensearch.md &sect;14). Durably pins every shard's current manifest generation
 * under {@code snapshot_id}, all-or-nothing.
 */
public class RestIndexSnapshotPinAction extends BaseRestHandler {

    /** Creates the handler; stateless, so no configuration is needed. */
    public RestIndexSnapshotPinAction() {}

    /** This handler's registered name, for logging/metrics. */
    @Override
    public String getName() {
        return "serverless_storage_index_snapshot_pin";
    }
     ...
     * @param client used to dispatch the parsed {@link IndexSnapshotPinRequest} locally.
     */
    @Override
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        ...
        IndexSnapshotPinRequest pinRequest = new IndexSnapshotPinRequest(indexName, (String) snapshotId);
        return channel -> client.executeLocally(IndexSnapshotPinAction.INSTANCE, pinRequest, new RestToXContentListener<>(channel));
    }
}
