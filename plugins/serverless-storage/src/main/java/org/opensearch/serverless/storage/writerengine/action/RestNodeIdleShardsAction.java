...
/**
 * {@code GET /_plugins/_serverless/storage/_idle_shards} -- the REST surface for {@link
 * NodeIdleShardsAction}. No path or body parameters: lists every writer shard's idle time tracked
 * on whichever node answers the request, matching {@link RestShardIdleTimeAction}'s own "answers
 * only from the receiving node" contract -- a caller wanting cluster-wide idle-shard data is
 * expected to call this once per data node, same as that action.
 */
public class RestNodeIdleShardsAction extends BaseRestHandler {

    /** Creates the handler; stateless, so no configuration is needed. */
    public RestNodeIdleShardsAction() {}

    /** This handler's registered name, for logging/metrics. */
    @Override
    public String getName() {
        return "serverless_storage_node_idle_shards";
    }
    ...
    public List<Route> routes() {
        return singletonList(new Route(GET, "/_plugins/_serverless/storage/_idle_shards"));
    }

    /**
     * @param request the incoming REST request; unused beyond routing, this action takes no parameters.
     * @param client used to dispatch the parsed {@link NodeIdleShardsRequest} locally.
     */
    @Override
    protected RestChannelConsumer prepareRequest(RestRequest request, NodeClient client) throws IOException {
        return channel -> client.executeLocally(
            NodeIdleShardsAction.INSTANCE,
            new NodeIdleShardsRequest(),
