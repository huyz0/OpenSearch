...
/**
 * Proves {@link RestIndexSnapshotReleaseAction#prepareRequest} correctly parses a well-formed
 * body and rejects a malformed one, mirroring {@code RestShardCloneActionTests}'s own shape.
 ...
 * ServerlessStorageIndexSnapshotActionIT}); this only proves body-parsing correctness in
 * isolation.
 */
public class RestIndexSnapshotReleaseActionTests extends OpenSearchTestCase {

    private final RestIndexSnapshotReleaseAction action = new RestIndexSnapshotReleaseAction();
    ...
    private static RestRequest requestWithBody(String json) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withContent(new BytesArray(json), XContentType.JSON)
            ...
            .withPath("/_plugins/_serverless/storage/index/my-index/_snapshot_release")
            .withParams(Collections.singletonMap("index", "my-index"))
            .build();
    }

    public void testPrepareRequestParsesAWellFormedBody() throws Exception {
        RestRequest request = requestWithBody("{\"snapshot_id\":\"snap-1\"}");
    ...
    public void testPrepareRequestRejectsANonStringSnapshotId() {
        RestRequest request = requestWithBody("{\"snapshot_id\":123}");
        IllegalArgumentException e = expectThrows(IllegalArgumentException.class, () -> action.prepareRequest(request, null));
        assertTrue(e.getMessage().contains("snapshot_id"));
    }
}
