...
/**
 * Proves {@link RestSnapshotReleaseAction#prepareRequest} correctly parses a well-formed body
 ...
 * ServerlessStorageSnapshotPinActionIT}, which also exercises release as the pin/release
 * lifecycle); this only proves body-parsing correctness in isolation.
 */
public class RestSnapshotReleaseActionTests extends OpenSearchTestCase {

    private final RestSnapshotReleaseAction action = new RestSnapshotReleaseAction();
    ...
    private static RestRequest requestWithBody(String json) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withContent(new BytesArray(json), XContentType.JSON)
            ...
            .withPath("/_plugins/_serverless/storage/_snapshot_release")
            .build();
    }
