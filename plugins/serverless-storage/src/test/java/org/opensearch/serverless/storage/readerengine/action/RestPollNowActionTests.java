...
/**
 * Proves {@link RestPollNowAction#prepareRequest} correctly parses path parameters from a
 ...
 * to end by {@code ServerlessStoragePublicationNotificationIT}); this only proves path-parsing
 * correctness in isolation.
 */
public class RestPollNowActionTests extends OpenSearchTestCase {

    private final RestPollNowAction action = new RestPollNowAction();
    ...
    private static RestRequest requestWithParams(Map<String, String> params) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withMethod(RestRequest.Method.POST)
            .withPath("/_plugins/_serverless/storage/{index_uuid}/{shard_id}/_poll_now")
            .withParams(params)
            .build();
    }
