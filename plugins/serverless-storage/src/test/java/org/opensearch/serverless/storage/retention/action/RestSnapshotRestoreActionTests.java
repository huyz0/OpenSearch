...
/**
 * Proves {@link RestSnapshotRestoreAction#prepareRequest} correctly parses a well-formed body
 ...
 * ServerlessStorageSnapshotRestoreActionIT}); this only proves body-parsing correctness in
 * isolation.
 */
public class RestSnapshotRestoreActionTests extends OpenSearchTestCase {

    private final RestSnapshotRestoreAction action = new RestSnapshotRestoreAction();
    ...
    private static RestRequest requestWithBody(String json) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withContent(new BytesArray(json), XContentType.JSON)
            ...
            .withPath("/_plugins/_serverless/storage/_snapshot_restore")
            .build();
    }
