...
/**
 * Proves {@link RestRealtimeGetAction#prepareRequest} correctly parses path parameters from a
 ...
 * client.executeLocally} (that requires a real {@link NodeClient}/transport); unlike this
 * plugin's other actions, no dedicated {@code *IT} currently exercises this endpoint's real
 * dispatch, so this class is the only coverage {@link RestRealtimeGetAction} has today.
 */
public class RestRealtimeGetActionTests extends OpenSearchTestCase {

    private final RestRealtimeGetAction action = new RestRealtimeGetAction();
    ...
    private static RestRequest requestWithParams(Map<String, String> params) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withMethod(RestRequest.Method.GET)
            .withPath("/_plugins/_serverless/storage/{index_uuid}/{shard_id}/_realtime_get/{id}")
            .withParams(params)
            .build();
    }
