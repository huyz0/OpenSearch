...
/**
 * Proves {@link RestCompactionTriggerAction#prepareRequest} correctly parses a well-formed body
 ...
 * ServerlessStorageCompactionTriggerActionIT}); this only proves body-parsing correctness in
 * isolation.
 */
public class RestCompactionTriggerActionTests extends OpenSearchTestCase {

    private final RestCompactionTriggerAction action = new RestCompactionTriggerAction();
    ...
    private static RestRequest requestWithBody(String json) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withContent(new BytesArray(json), XContentType.JSON)
            ...
            .withPath("/_plugins/_serverless/storage/_compact")
            .build();
    }
