...
/**
 * Proves {@link RestNodeWalBacklogAction#prepareRequest} builds cleanly on a bare GET request --
 ...
 * already covered end to end by {@code ServerlessStorageNodeWalBacklogActionIT}); this only
 * proves request-building correctness in isolation.
 */
public class RestNodeWalBacklogActionTests extends OpenSearchTestCase {

    private final RestNodeWalBacklogAction action = new RestNodeWalBacklogAction();
    ...
    private static RestRequest bareRequest() {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withMethod(RestRequest.Method.GET)
            .withPath("/_plugins/_serverless/storage/_wal_backlog")
            .build();
    }
