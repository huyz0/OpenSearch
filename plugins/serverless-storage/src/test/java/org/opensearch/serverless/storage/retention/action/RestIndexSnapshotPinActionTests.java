...
/**
 * Proves {@link RestIndexSnapshotPinAction#prepareRequest} correctly parses a well-formed body
 * and rejects a malformed one, mirroring {@code RestShardCloneActionTests}'s own shape.
 ...
 * isolation.
 */
public class RestIndexSnapshotPinActionTests extends OpenSearchTestCase {

    private final RestIndexSnapshotPinAction action = new RestIndexSnapshotPinAction();
    ...
    private static RestRequest requestWithBody(String json) {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withContent(new BytesArray(json), XContentType.JSON)
            ...
            .withPath("/_plugins/_serverless/storage/index/my-index/_snapshot_pin")
