...
/**
 * Proves {@link RestNodeManifestLagAction#prepareRequest} builds cleanly on a bare GET request --
 ...
 * already covered end to end by {@code ServerlessStorageNodeManifestLagActionIT}); this only
 * proves request-building correctness in isolation.
 */
public class RestNodeManifestLagActionTests extends OpenSearchTestCase {

    private final RestNodeManifestLagAction action = new RestNodeManifestLagAction();
    ...
    private static RestRequest bareRequest() {
        return new FakeRestRequest.Builder(NamedXContentRegistry.EMPTY).withMethod(RestRequest.Method.GET)
            .withPath("/_plugins/_serverless/storage/_manifest_lag")
            .build();
    }
