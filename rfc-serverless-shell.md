...
| **M14** | ⏳ **PARTLY DONE.** **The shell runs on an object store** (`serverless.store.type: s3`, reached through `S3RepositoryPlugin#getRepositories` rather than package-private internals): a node boots against a bucket, publishes its commit, renews its lease and answers searches, and a successor with its own `path.home` rebuilds from the bucket alone. Doing it found a bug no filesystem test could — `IndexInputStream` had no mark support, which the S3 client requires to retry, so **publishing a segment was impossible on S3 and worked perfectly on disk**. Lazy block-range reads are done. See [`m14-object-store-notes.md`](m14-object-store-notes.md). **Remaining:** sharded GC sweep; name index / prefix search; shard-count scaling and per-index memory, still unmeasured; nothing measured on S3 at all; contention and object storage never combined. | M11 |
