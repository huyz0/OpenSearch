...
> the largest gap is that the data path still runs on a local filesystem: **R11 now passes against MinIO**,
> but the shell itself has never been pointed at an object store, and MinIO is not AWS S3. The green table below is not
> the whole picture.
>
