...
> **D4, as operated (2026-09).** Only the prohibitive half of D4 has been exercised: `server/` has zero
> changed files and the build fails if it references `serverless`. The permissive half — the four shared
> interfaces of §8.1 — was never built, and `serverless-status.md` for a time restated the pre-D4 rule
> ("out of scope while `server/` may not change") and blamed three limitations on it. The decision stands
> as written; what changes is the honesty of the risk table: R6 is Medium until §8.1 exists, and the three
> limitations are open work under D4, not consequences of a rule.
>
> **D5, as operated (2026-09).** The daemon boots on S3 (`ObjectStores`) and the conformance suite has run
> against MinIO and SeaweedFS but not against a provider account; R11 is unclosed. Startup on a non-filesystem
> store therefore logs the R11 status and the providers actually tested, which is the "flag that says so".
...
- **Data-plane actions** (`TransportShardBulkAction`, the search phases, get/mget, TransportReplicationAction
  ...
  **Reused unmodified.** This is the bulk of the request path and the reason this plan is affordable.
  ...
  > **Amended 2026-09 (R3 realised).** This did not happen. None of those classes is used: each assumes a
  > replication group, a `ClusterState` routing table with replicas, and a transport action registry the
  > shell does not build. The shell has its own request path — `shard/ShardOperations`, `rest/SearchFanout`,
  > `rest/BulkHandler`, `transport/ShardRouter` and the `Forwarded*` messages — about 19.6k lines against the
  > ~5k this section budgeted for the whole shell. The allowlist capped the *surface*, not the *cost*. What
  > is still reusable and should be adopted next is the static primary-side entry point
  > `TransportShardBulkAction.performOnPrimary(...)`, which takes an `IndexShard` and a mapping-update
  > callback and would replace the shell's own `op_type`, version and `retry_on_conflict` handling.
...
> **Amended 2026-09.** M63 added `/cluster/members` beside the leases: an *index* of member ids, so that a
> refresh is one read rather than a listing. It is a deviation from the sentence above and is accepted on
> these terms only: it is a cache of the listing and never the truth — a refresh that finds it absent, or a
> node that finds itself missing from it, rebuilds it from a listing; liveness still comes from each
> node's own lease register; and its writer population is bounded by write *rate*, not by node count —
> one compare-and-swap per join and per clean leave, retried in the background, never on the per-pass
> path. If a join storm ever makes that swap contend, the index is to be sharded by `hash(nodeId)`, not
> given a retry loop. The safety argument does not depend on it.
...
| R3 | `action/` re-implementation is larger than §6.3 assumes | **Realised** | Allowlist + 501 capped the surface, not the cost: ~19.6k lines of request path exist (§6.3 amendment). Mitigation now is adoption of `TransportShardBulkAction.performOnPrimary` and deletion of the duplicated write semantics, one path at a time |
...
| R6 | Upstream drift in the data plane's internal APIs | Medium | §8.1's extraction was never done (see the D4 note in §1.1); the shell still passes a whole `ClusterService` into `IndicesService`. The downgrade to Low is withdrawn until the four interfaces exist in `server`. D4 rule 1 (server never references serverless) is CI-enforced |
