...
| **M15** | **Production surface** | Auth and multi-tenancy — today anyone who can reach the port can delete an index. Classic create-body envelope, or an explicit decision that D2 makes it a non-goal. Delete draining a live writer. Document deletes are done; delete-by-query, bulk delete and version-conditional delete are not, and the GC sweep has not been re-examined against tombstones. The `cluster/config` register, named in §9.3 and never built. | M11 |
