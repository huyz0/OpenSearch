...
> scheduling half of **M13** are delivered — a document can be written to any node and found from any
> node in a multi-node deployment, it survives its node dying, and a node now renews, publishes and
> reacquires without being driven. **§12.1 lists what remains**; the largest gap is that **M12/R11 has
> never been run**, so none of this has been shown on real object storage, and the shell still has no
> `main()` — nodes are constructed by tests. The green table below is not the whole picture.
>
...
| **M12** | ⚠️ **SUITE WRITTEN, NOT RUN ON A PROVIDER.** Eight properties the design depends on are now tests, including ranged-read exactness and listing length accuracy — neither previously written down. Self-checked against planted defects so it cannot pass vacuously. Runs against `FsBlobContainer`; **no S3 binding**, blocked on `S3BlobStore`'s package-private 17-argument constructor rather than on credentials. See [`r11-conformance.md`](r11-conformance.md). Remaining: | CAS linearizability under contention on S3 and GCS; listing bounded by `start-after`/`max-keys`; clock skew between a writer and its successor. | — |
| **M13** | ⚠️ **SCHEDULING DONE; THE REST OPEN.** A node runs by itself: **three drivers, not one timer** — lease renewal on a clock at `ttl/3` (the only genuinely clock-bound job), publication **edge-triggered by a write and coalesced** (200 writes → 1 publish), activation **triggered by failure** (a fenced publish, an unreachable owner, a write to an unowned shard). A slow backstop runs the full `tick` regardless, so the edges are accelerators and the timer is the guarantee. `tick()` is unchanged in behaviour, now a composition of four separately-callable passes. Eleven canaries planted and caught. **Remaining:** no `main()` — `startFor` is what a bootstrap would call and nothing outside a test does; no reactivation beyond the `wanted` set; no scale-to-zero controller; two liveness modes still coexist; no jitter, so co-started nodes renew in lockstep; intervals are reasoned, not measured. See [`m13-notes.md`](m13-notes.md). | M11 |
| **M14** | **Scale hardening** | Sharded GC sweep; name index / prefix search (`plan-area-a-name-index.md`); shard-count scaling and per-index memory, neither yet measured. **Lazy block-range reads are done** — a reader fetches 16% of a shard to open and answer a query, and 0 bytes on a repeat; see [`block-reads-notes.md`](block-reads-notes.md). | M11 |
