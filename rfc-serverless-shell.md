...
| **M15** | **Production surface** | Auth and multi-tenancy — today anyone who can reach the port can delete an index. Classic create-body envelope, or an explicit decision that D2 makes it a non-goal. Delete draining a live writer. Bulk index and delete are done; document deletes are done, and the GC sweep has been re-examined against tombstones — tombstones ride the manifest, so the existing rule already covers them, and the log is reclaimed by the publish path rather than by the sweep ([`m15-delete-notes.md`](m15-delete-notes.md)). Delete-by-query, bulk delete and version-conditional delete are not. The `cluster/config` register, named in §9.3 and never built. | M11 |
