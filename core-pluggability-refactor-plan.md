...
- [ ] `AbsentIndexRoutingSuppliers.java` and `AbsentIndexDescriptorSuppliers.java` no longer exist in `server/` (Phase C4). **Not yet — C5 (the plugin registration these call sites need to migrate onto) is now done and verified; C4a's call-site migration itself has not started.**
...
| C4a | **in progress: 12/28 call sites migrated across 6 files, each its own commit, each individually verified** | C2's real fix (see the C5 row) means a caller's existing `metadata.index(...)` never changes behavior no matter what's registered — so migrating a call site now means deliberately changing it to call `metadata.indexOrResolved(...)` instead, an explicit, auditable, one-line diff per call site, not a silent behavior change lying in wait for whenever a resolver happens to get attached. Landed so far, each with its own commit and a real `internalClusterTest` run (not just `:server:test`): `MetadataDeleteIndexService#deleteIndices` (the one that proved the C5 regression — now correctly split between the plain `index()` gated-check and the explicit `indexOrResolved()` fetch), `TransportGetIndexAction` (SETTINGS + CONTEXT branches), `TransportGetSettingsAction`, `TransportGetAction#getExecutor`, `TransportBulkAction` (all 5 of its call sites), `TransportReplicationAction` (2 of its 3 — its third, `AbsentIndexRoutingSuppliers.resolveShard`, is `resolveShard`, not `resolve`/`metadataOrDescriptor`, and its own javadoc documents a previously-fixed bug from collapsing a published/absent distinction `ClusterState#getIndexRoutingTable` doesn't preserve today, so it correctly belongs in C4b instead — caught by reading the file rather than pattern-matching the method name). Removed the now-unused `AbsentIndexDescriptorSuppliers` import from each migrated file. **16 call sites remain**: `TransportShardBulkAction`, `TransportBroadcastReplicationAction`, `TransportSingleShardAction`, `TransportUpdateAction`, `TransportAnalyzeAction`, `TransportGetFieldMappingsIndexAction`, `ClusterStateHealth`, `OperationRouting`, `ShardPaginationStrategy`, `ActiveShardCount`, `IndicesClusterStateService`, `SnapshotsService` — continue one file at a time, each its own commit, each verified against a real `internalClusterTest`, and each read carefully rather than assumed safe from the method name matching (as `TransportReplicationAction` just demonstrated). |
| C4b | not started | The other ~27 call sites (`allShards`/`supply`/`gatedAmong`/`exists`/`shouldPublishRouting`/prefix-expansion/pagination/registration-listeners) still need the SPI's surface expanded first, as originally found — `indexOrResolved`'s explicit-opt-in shape doesn't change that, since these operations don't fit a single-index resolve call at all. |
| D1-D3 | not started | |
| E1-E2 | not started | |
| F | not started | |
