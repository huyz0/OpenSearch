...
| C4-C5 (full migration) | not started — precisely scoped, **and a real ordering bug in this plan itself caught before any call site was touched** | While preparing to migrate the 28 "safe" `metadataOrDescriptor`/`resolve` call sites identified in the C4 row above, checked what `plugins/serverless-storage` (the only real consumer) actually registers against today: `DescriptorGate`/`ComputedPlacementGate`/`GatedShardSuspensionRegistry` all call the **static registries' own `.register(...)` methods directly** — nothing in the plugin implements `getIndexMetadataResolver()`/`getIndexRoutingResolver()` yet, because C5 (updating the plugin to register through the new `ClusterPlugin` SPI) has not happened. This means `Metadata#index`/`ClusterState#getIndexRoutingTable`'s `resolver` field is `null` on every real cluster running this plugin today. **The plan's stated order — C4 (migrate call sites) then C5 (migrate the plugin) — is backwards and would have caused a real regression if followed as written**: migrating a call site from `AbsentIndexDescriptorSuppliers.metadataOrDescriptor(...)` to the plain `metadata.index(...)` accessor *before* the plugin also registers through the new SPI would silently stop that call site from resolving any gated index at all on a real cluster — the exact feature this whole area exists to serve, broken for whichever call sites got migrated first. Caught here, before any call site was actually touched, by checking the real consumer's current registration path rather than assuming the new SPI was already reachable. **Corrected order: C5 must land and be verified first, then C4.** C5 itself was investigated for feasibility this session and deliberately not attempted blind: `DescriptorGate.java` alone is real, carefully-tuned production code (wildcard expansion limits re-measured against a "wake storm," a documented three-times-shipped "mechanism built and never connected" failure mode, tombstone/prefetch/freshness semantics) with its own `internalClusterTest` suite that this session never ran even once — writing a new `IndexMetadataResolver` adapter that delegates into it, and trusting the result without that suite in hand, is exactly the kind of unverified change the ground rules exist to prevent. A purely-additive version (the plugin registers via *both* the old static path and the new SPI simultaneously, changing nothing about existing behavior since nothing new calls the SPI path until C4 lands) is the safe shape for C5 to take — sketched here for whoever picks this up, not implemented. |
| D1-D3 | not started | |
| E1-E2 | not started | |
| F | not started | |
