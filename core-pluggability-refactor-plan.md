...
| C4a | **unblocked, by design rather than by luck** | C2's real fix (see the C5 row) means a caller's existing `metadata.index(...)` never changes behavior no matter what's registered — so migrating a call site now means deliberately changing it to call `metadata.indexOrResolved(...)` instead, an explicit, auditable, one-line diff per call site, not a silent behavior change lying in wait for whenever a resolver happens to get attached. Migrate the 28 `metadataOrDescriptor`/`resolve` call sites, one file at a time, each its own commit, using `indexOrResolved(...)`/`getIndexRoutingTable(...)` in place of the static registry calls. Still verify each on a real multi-node `internalClusterTest` — `MetadataDeleteIndexService` proved once already in this session that a call site can look obviously equivalent on paper and be wrong in a way only a real cluster test catches. |
