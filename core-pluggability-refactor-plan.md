...
| C4a | **in progress: 5/28 call sites migrated, each its own commit, each individually verified** | C2's real fix (see the C5 row) means a caller's existing `metadata.index(...)` never changes behavior no matter what's registered — so migrating a call site now means deliberately changing it to call `metadata.indexOrResolved(...)` instead, an explicit, auditable, one-line diff per call site, not a silent behavior change lying in wait for whenever a resolver happens to get attached. Landed so far, each with its own commit and a real `internalClusterTest` run (not just `:server:test`): `MetadataDeleteIndexService#deleteIndices` (the one that proved the C5 regression — now correctly split between the plain `index()` gated-check and the explicit `indexOrResolved()` fetch), `TransportGetIndexAction` (SETTINGS + CONTEXT branches), `TransportGetSettingsAction`, `TransportGetAction#getExecutor`. Removed the now-unused `AbsentIndexDescriptorSuppliers` import from each migrated file. **23 call sites remain** across `TransportBulkAction` (6), `TransportReplicationAction` (2), `TransportShardBulkAction`, `TransportBroadcastReplicationAction`, `TransportSingleShardAction`, `TransportUpdateAction`, `TransportAnalyzeAction`, `TransportGetFieldMappingsIndexAction`, `ClusterStateHealth`, `OperationRouting`, `ShardPaginationStrategy`, `ActiveShardCount`, `IndicesClusterStateService`, `SnapshotsService` — continue one file at a time, each its own commit, each verified against a real `internalClusterTest`, not assumed safe from the pattern matching. |
