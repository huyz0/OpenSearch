...
- [x] Every phase A-H item is either landed on `refactor/core-pluggability`, or has an explicit, written reason in this document for why it's deferred (not silently dropped). **A, B, G, H fully landed. C is landed through C1-C3 plus the `Node.java` attachment point (the full, plugin-reachable plumbing, not just the additive half); C4/C5 and D/E are next, now unblocked. F is deferred with a written reason (needs deeper `DataFormatRegistry` investigation than this session could safely do).**
...
| C5 | **done, additively** | Added two generic, plugin-agnostic adapters in `server/` — `SupplierBackedIndexMetadataResolver`/`SupplierBackedIndexRoutingResolver` — that implement the new SPI by delegating straight back to the pre-existing static registries (`AbsentIndexDescriptorSuppliers#synthesisedMetadata`, `AbsentIndexRoutingSuppliers#supply`/`shouldPublishRouting`/`localShards`). Pure, stateless forwarding — zero new lookup/caching logic — so fully unit-tested with `:server:test` alone, without needing `plugins/serverless-storage`'s own test infrastructure at all, and reusable by any future plugin that populates the same registries. Wired `ServerlessStoragePlugin.getIndexMetadataResolver()`/`getIndexRoutingResolver()` to return them, unconditionally and additively: `DescriptorGate.install` still registers with the static registry exactly as before, this only adds a second path to the same answer, and resolves to `null` (identical to today) wherever nothing is registered in the static registry yet (before install, or with gating disabled). Verified beyond `:server:compileJava`/`test`: ran `:plugins:serverless-storage:compileJava`/`compileTestJava` (clean) and its full `:plugins:serverless-storage:test` suite **twice** (1341 tests) — the first run had one unrelated failure (`StatsProjectingMappingStoreTests`, a thread-pool-drain timing test with zero relation to this change), which repeated isolated runs showed was absent both with and without this change, confirming pre-existing flakiness rather than a regression; the second full run passed 1341/1341 clean. `plugins/serverless-storage`'s own `internalClusterTest` suite was not run (too slow for this session's remaining budget) — flagged for whoever verifies C4a next. |
| C4a | not started, now unblocked | Migrate the 28 `metadataOrDescriptor`/`resolve` call sites identified in the C4 row, one file at a time, each its own commit, now that C5 makes `metadata.index(name)`/`state.getIndexRoutingTable(name)` actually reach a working resolver on a real cluster. Verify each on a real multi-node `internalClusterTest`, not just `:server:test` — cluster-state propagation timing matters here, and this session never ran `plugins/serverless-storage`'s own `internalClusterTest` suite at all, so treat the first migrated call site's verification as also the first real end-to-end proof this phase's design works, not a formality. |
| C4b | not started | The other ~27 call sites (`allShards`/`supply`/`gatedAmong`/`exists`/`shouldPublishRouting`/prefix-expansion/pagination/registration-listeners) need the SPI's surface expanded first — a real design effort of its own, not a mechanical migration. See the C4 row for the specific gaps. |
| D1-D3 | not started | |
| E1-E2 | not started | |
| F | not started | |
...
phase, plus a deadlock gap the attach point's own tests found and closed); I mostly done on its own branch.**
Every commit landed here compiles clean and has its existing and/or new test suite passing — nothing in this
branch is speculative or unverified. D, E, F, and C4/C5 are not started, each for a specific, written reason
(usually: the remaining work needs either a live multi-node cluster or the plugin's own test suite to verify
safely, or deeper investigation of a subsystem — `DataFormatRegistry` for F — than this session could
responsibly do blind.
...
2. ~~Harden the resolver contract against the cluster-state-thread deadlock~~ — **done**:
   ...
   status-log row.
3. **C5 before C4 — the plan's original order was backwards, and this session caught it before acting on it.**
   See the "C4-C5 (full migration)" status-log row for the full reasoning: `plugins/serverless-storage`
   registers against the *static* registries only today, so migrating a call site to the plain accessor
   before the plugin also speaks the new SPI would silently break resolution for that call site on a real
   cluster. Do C5 first — additively (register through `getIndexMetadataResolver()`/`getIndexRoutingResolver()`
   *alongside* the existing static registration, not instead of it, so nothing existing changes) — and run
   `plugins/serverless-storage`'s own `internalClusterTest` suite, not run at all this session, before trusting
   the result.
4. **C4a**: once #3 is verified, migrate the 28 `metadataOrDescriptor`/`resolve` call sites (the ones that
   already match the SPI's shape) off the static registries, one file at a time, each its own commit. Verify
   on a real multi-node `internalClusterTest`, not just `:server:test` — cluster-state propagation timing
   matters here.
...
6. **D and E**: unblocked once C4/C5 land, following the plan's existing sections.
