...
as C1, with one real API-annotation gotcha found and resolved along the way — see the D1 status-log row); I
mostly done on its own branch.** Every commit standing on this branch compiles clean and
has its test suite passing, including the exact `internalClusterTest` that caught the C5 regression, now
re-run and green, and a real `internalClusterTest` run per C4a commit. D2/D3, E, F's last site, C4b, and
Phase I's 3 items are not started, each for a specific, written reason.
