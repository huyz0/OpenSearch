...
6. ~~D1 (define the `IndexCreationStrategy` SPI)~~ — **done**, in a later continuation of this session, after
   ...
   annotating).
7. **D2/D3, and E — still deliberately NOT started.** Both are explicitly flagged in their own plan sections
   as needing time this session cannot honestly provide: D2 is "risk: medium-high... do this after C is fully
   landed and **soaked**" (touches five core services' primary entry points, removes a field from a shared
   DTO); E is "risk: high... **the largest single extraction in the plan**... expect it to be several weeks of
   a human team's time... consider feature-flagging." C landed *minutes* before this document was first
   written in this continuation — essentially zero soak time by any real-world definition (this is agent
   session time, not calendar time an operator has actually run C in production). Attempting either blind,
   immediately after the C5 saga demonstrated exactly how a change that passes every available check can
   still hide a severe regression, would repeat the mistake this session just spent real effort learning not
   to make. Do not start D2 or E without a genuine gap after C has run in a real environment, and do not skip
   D2's five-service call-site audit by analogy to how quickly C4a or D1 went — both of those were small,
   individually-verified, additive changes to an already-designed seam; D2 is migrating five services'
   primary entry points onto a *brand-new* seam whose real signature (per D1's own status-log note) is still
   unverified against actual callers.
