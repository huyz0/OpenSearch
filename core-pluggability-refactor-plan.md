...
| C4-C5 (full migration) | not started, and now precisely scoped rather than blocked — see the C4 row above for why a blind "migrate ~25 call sites" pass was replaced with this investigation instead | |
| D1-D3 | not started | |
| E1-E2 | not started | |
| F | not started | |
...
2. **C4** (now unblocked): migrate the ~25 call sites off the static `AbsentIndex*Suppliers` registries, one
   file at a time, each its own commit. Verify on a real multi-node cluster (an `internalClusterTest`, not
   just unit tests — cluster-state propagation timing matters here), not just `:server:test`.
