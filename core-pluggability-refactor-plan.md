...
F now 4 of 5 sites done (`TextFieldMapper`'s stored-field forcing landed in a later continuation — the
"unresolved `FieldType` mutation" blocking it was a red herring, a field-hiding trap in the diagnostic code,
not the fix; a second real bug, a behavior-flipping wrong default, was caught by manual review before commit,
not by any test, since `server/`'s own suite structurally cannot exercise the branch that bug lived in — see
the F status-log row for the full arc); I mostly done on its own branch.** Every commit standing on this
branch compiles clean and has its test suite passing, including the exact `internalClusterTest` that caught
the C5 regression, now re-run and green, a real `internalClusterTest` run per C4a commit, and (for the F
fix) a `-Dsandbox.enabled=true` run of all three real `DataFormat` implementations' own test suites, since
that was the only way to verify the branch `server/`'s tests can't reach. D2/D3, E, `SourceFieldMapper`
(F's one remaining site), C4b, and Phase I's 2 remaining items are not started, each for a specific, written
reason.
...
7. ~~F~~ — **4 of 5 sites done.** `ObjectMapper.parseNested`, `KeywordFieldMapper.PARSER`,
   `DocumentParser.builderSupplierForText`, and (in a later continuation) `TextFieldMapper`'s stored-field
   forcing are all landed and verified — see the F status-log row for the full arc, including a genuinely
   interesting resolution: the "unresolved `FieldType` mutation" that blocked `TextFieldMapper` was a Java
   field-hiding trap in the *diagnostic* code, not a real mutation, and a second real bug (a
   behavior-flipping wrong default) was only caught by manual review, not by any test, because `server/`'s
   own suite structurally cannot exercise the branch that bug lived in. **`SourceFieldMapper`'s analogous
   stored-field forcing is the only site left** — it needs its own constructor/merge-builder plumbing built
   from scratch (unlike `TextFieldMapper`, it has no existing multi-arg `Builder` to extend), but is
   otherwise unblocked; reuse `TextFieldMapper#requiresStoredFieldsForPluggableFormat`'s shape rather than
   re-deriving it, and budget for the same "manually re-check the chosen default's polarity against the code
   it replaces, don't trust that a plausible-sounding default is behavior-preserving" step that caught the
   second bug above.
