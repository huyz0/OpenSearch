...
| D1-D3 | not started | |
| E1-E2 | not started | |
| F | **1 of 5 call sites done and verified (`ObjectMapper.parseNested`); 4 remain, correctly re-scoped as needing real `Builder`-construction plumbing, not a mechanical pass** | Investigated via a dedicated agent first. Confirmed real, useful facts: `DataFormat`/`DataFormatRegistry`/`DataFormatPlugin` are upstream `main`'s own in-flight `@ExperimentalApi` feature (not a serverless invention), and `isPluggableDataFormatEnabled` at the 19 call sites splits into (A) 14 clean, additive, format-independent engine/write-path dispatch sites needing no change, and (B) 5 genuine per-format capability decisions collapsed onto one global boolean — plus a correction to the plan's own original text: `RoutingFieldMapper.preParse` is actually category (A), not (B). **Attempted the investigation's recommended first step** (`DataFormat.requiresStoredFields()` on `TextFieldMapper`/`SourceFieldMapper`) and found it doesn't fit as described: `Mapper.BuilderContext` has no `DataFormatRegistry` access at all (only `ParserContext` does), and the cited `KeywordFieldMapper.PARSER` precedent turned out to read the same raw global flag this phase is trying to move away from, not a capability — so there's no existing working example of a `Builder` built from a capability query, and `SourceFieldMapper.Builder` has no non-default constructor to extend. Reverted that speculative addition rather than land it unwired. **Landed the actually-cheapest site instead**: `ObjectMapper.TypeParser.parseNested`, which already runs inside `ParserContext` with no new `Builder`-construction plumbing needed. Added `DataFormat.unsupportedMapperTypes()`, defaulting to `Set.of("nested")` — chosen specifically so it exactly matches today's universal behavior and needs zero changes to any existing concrete `DataFormat` (`ParquetDataFormat`, `CompositeDataFormat`, `LuceneDataFormat`, test stubs) to stay behavior-preserving, unlike `requiresStoredFields()`'s `false` default, which would have silently regressed every format. **A real bug surfaced by the new test itself, not just reasoning**: the first implementation treated a null `DataFormatRegistry` as "nested is fine" on the theory that a null registry only ever means "feature is off" — wrong, because `ObjectMapperPluggableDataFormatTests` (a lightweight mapper-only test harness that builds `MapperService` directly, bypassing `DocumentMapperParser`'s registry-threading) has the feature on via settings with no registry wired at all, and the same gap could exist for any caller outside the exact production construction path. The test caught it immediately (silently stopped rejecting nested); fixed by falling back to the original flag check (`isPluggableDataFormatEnabled(...)`) whenever the registry is null, rather than assuming null means the feature is off. New `ObjectMapperPluggableDataFormatTests` (1 test, pinning down exactly the scenario that caught the bug). Verified: `:server:compileJava`/`compileTestJava` clean; the whole `org.opensearch.index.mapper.*` test package (every mapper test in `server/`) passes. **What's left**, corrected for whoever continues: `TextFieldMapper`/`SourceFieldMapper` stored-field forcing, `KeywordFieldMapper`'s raw-value `PARSER`, `DocumentParser.builderSupplierForText` — each needs a `DataFormatRegistry`-derived value computed at `TypeParser.parse(...)` time (where `ParserContext` is available) and threaded into that mapper's `Builder` constructor, a real plumbing change per site (small for `TextFieldMapper`, which already threads `ParserContext` into its `Builder`; a genuine new constructor for `SourceFieldMapper`, which currently has none). Budget one commit per site, each with its own `Builder`-construction test modeled on `ObjectMapperPluggableDataFormatTests` — including, critically, a test for the "registry null but feature on" case each time, since that's exactly the case a first attempt is likely to get wrong again. |
