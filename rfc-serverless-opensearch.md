...
bullets of this section.** Rejects `index.serverless_storage.enabled=true` at index creation time
unless the node has `cluster.remote_store.state.enabled=true` (core's
`RemoteClusterStateService.REMOTE_CLUSTER_STATE_ENABLED_SETTING`, a `Property.Final` node-level
flag). Verified with unit tests (rejects when disabled, allows when enabled, ordinary
non-serverless indices unaffected either way) confirmed meaningful by reverting and watching the
new tests fail to compile -- and, since this check runs on every real index creation, by a full
`internalClusterTest` sweep (every one of this plugin's ~26 IT classes creates at least one
serverless-storage index): the first sweep attempt caught a real, broad regression this change
introduced (every one of those IT classes failed, since none of their test clusters had remote
cluster state enabled), fixed by adding a shared `ServerlessStorageIntegTestCase` base class (and,
for the four IT classes that couldn't extend it because they already extend core's
...
lazy-directory and existing-shards-allocator settings and rejects the conflicting replication-type
combo above -- since that interface only ever sees per-request index settings, never node-level
ones, the node's resolved remote-cluster-state value had to be smuggled in via a late-setter
mutable field (`setRemoteClusterStateEnabled`, wired from `ServerlessStoragePlugin#createComponents`),
directly contradicting that class's own "no configuration state" javadoc and forcing every
pre-existing test in its suite to route through a special constructor helper first. Moved to a new
`ServerlessStorageRemoteClusterStateValidator implements IndexCreationValidator`, registered via
`Plugin#getIndexCreationValidators()`: `IndexCreationValidator#validate` receives a real
`IndexSettings` per call, and `IndexSettings#getNodeSettings()` already exposes real node settings
with no injection, no ordering hazard, and no mutable state needed at all, so the new validator is
a genuinely stateless function of its arguments and `ServerlessStorageIndexSettingProvider` is back
to being the pure function of request settings its own javadoc always claimed. Verified: reverting
the new validator class fails the plugin's own compile (its registration site no longer resolves);
a full `internalClusterTest` sweep re-run clean after the migration, confirming every IT's index
creation still succeeds under the new enforcement path.
...
**That open question is now investigated directly, in `ServerlessStorageOrdinaryStartupUnderHighLatencyIT`, with a real, somewhat surprising answer.** The 120 s failure above turned out not to indict *ordinary shard startup* itself: a brand-new, empty shard's own first-ever open under `LatencyProfile.HIGH` (`testOrdinaryShardStartupUnderSimulatedHighBlobLatency`) is cheap almost by construction -- one real run measured 821 ms and exactly 1 PUT + 1 GET, nowhere near the "dozens of small sequential blob writes" the earlier failure's own log suggested. That earlier 120 s failure most likely came from a different, heavier scenario this narrower test deliberately isolates away from (repeated suspend/reactivate cycling combined with ordinary startup in one shared budget), not from raw shard-open cost.

The genuinely representative half of the original concern -- a shard recovering *existing* durable data (real segment files, not an empty `Directory`) onto a fresh node -- is covered by a second test in the same class, `testShardRecoveryOntoAFreshNodeUnderSimulatedHighBlobLatency`, mirroring `ServerlessStorageWriterFailoverIT`'s own real cross-node-kill mechanics through this class's `HIGH`-latency-injecting repository instead of near-zero local disk. One real run of a single-document flushed shard measured 1696 ms and 1 PUT + 7 GETs on the survivor node to fully recover and become searchable again. Both numbers comfortably clear any reasonable bound for a minimal shard.

**The "does this scale to hundreds of segment files" half of that question is now answered too, by a third test in the same class, `testShardRecoveryWithManySegmentFilesUnderSimulatedHighBlobLatency`.** It forces a genuinely many-segment commit -- `index.merge.policy.segments_per_tier`/`max_merge_at_once` tuned generously high so TieredMergePolicy never consolidates below one segment per individually-refreshed document -- then flushes once (publishing all of them into a single manifest) before killing the primary and timing the survivor's recovery under `LatencyProfile.HIGH`. One real run with 100 pre-flushed un-merged segments measured 42985 ms and 1 PUT + 304 GETs (roughly 3 files/segment, consistent with `.si`/`.cfs`/`.cfe`) to fully recover, comfortably inside a generous 300 s bound reserved for this scenario. The real, honest conclusion: recovery cost scales linearly with segment count (materialization is a plain sequential per-file loop, so this isn't surprising in hindsight), with no non-linear blowup or failure mode found at this scale -- a long-lived, heavily-merged index with thousands of segments would extrapolate to minutes rather than tens of seconds under sustained `HIGH` latency, which is exactly the kind of real number this diagnostic exists to surface for capacity planning, not a correctness bug to fix.
