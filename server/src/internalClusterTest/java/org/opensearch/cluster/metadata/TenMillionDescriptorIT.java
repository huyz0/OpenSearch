...
/**
 * H21. The descriptor path at ten million indices.
 *
 * <p>The review closing the previous cycle said the distance between "no known ceiling" and "demonstrated"
 * needed a fleet. That is true of a hundred million and it is not true of ten. S27 sustained 18,853
 * creations per second at a million, so ten million is roughly nine minutes of writing on one machine, and
 * it puts the largest exercised population one order of magnitude from the target rather than two.
 *
 * <p>Flatness across a hundredfold increase from the original fifty thousand baseline is materially
 * stronger than flatness across twenty. And if the lookup bends between one million and ten, that bend is
 * the most valuable result this project could produce, because every extrapolation on the plan rests on it
 * not bending.
 *
 * <p>Everything below is inherited from H14, including the four harness failures recorded there. The
 * lesson that transfers is the fourth: {@code getTotalHits()} caps at 10,000, so the population check must
 * use {@code trackTotalHits(true)} or it verifies ten thousand documents and calls it ten million.
 *
 * <p>Original H14 rationale, kept because it is why this exists: every figure Area H had produced topped
 * out at fifty thousand. The curves are flat where flatness is the
 ...
 */
public class TenMillionDescriptorIT extends OpenSearchIntegTestCase {

    private static final String DESCRIPTORS = "descriptors";
    private static final int BASELINE = 1_000_000;
    ...
    public void testDescriptorPathAtAMillionIndices() throws Exception {
        ...
        report.append(
            String.format(
                Locale.ROOT,
                "%n  creation ratio %.2fx, lookup ratio %.2fx across a tenfold population increase%n",
