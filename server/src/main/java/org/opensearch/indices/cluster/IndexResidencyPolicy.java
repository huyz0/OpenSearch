...
/**
 * Phase E2 of {@code core-pluggability-refactor-plan.md}: an SPI a plugin implements to own the tuning of
 ...
 *
 * <p><b>What stayed in core, and why, after a dedicated E2 scoping investigation.</b> The plan's original
 ...
 * avoid the W4 deadlock -- a second, plugin-resident lifecycle that had to agree with this one on that
 ...
 * themselves -- stays in {@code IndicesClusterStateService}, unchanged.
 *
 ...
 * registration point Phase D1's {@link org.opensearch.cluster.metadata.IndexCreationStrategy} uses, and
...
public interface IndexResidencyPolicy {
     ...
     * from the heap using its own per-index cost estimate.
     *
     * <p>Defaults to zero -- derive it -- matching {@code IndicesClusterStateService}'s previous behavior.
     * A plugin that knows its own shard implementation's real per-index heap cost (the way {@code
     * GatedResidencySoakIT} measured 150,888 bytes for one specific implementation) should override this
     * rather than relying on a generic guess baked into core.
     */
    default int maxOpen() {
        return 0;
    }
