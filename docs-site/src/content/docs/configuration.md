...
All node settings are defined in `ServerlessStoragePlugin.java`. Most background machinery ships **disabled by default** — WAL batching in particular stays off even though the underlying group-commit mechanism is safe (see [Core Changes](/core-changes/)).
