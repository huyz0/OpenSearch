...
Resharding-by-copy auto-triggering, a general N-way merge (rather than a split's own two children merging back), and full snapshot-repository integration were each investigated and deliberately left out of scope — see [Resharding](/design/resharding/) and [Interoperability](/interoperability/) for what's actually implemented versus what was considered and rejected.
