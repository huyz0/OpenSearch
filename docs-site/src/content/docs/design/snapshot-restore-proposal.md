...
:::caution[Read the posture page first]
Nothing on this page is a backup in the sense of an independent copy that survives losing the source. [Durability Posture](/design/durability-posture/) says what each tier does and does not insure against, and what has to be configured on the object store instead.
:::
...
### A real byte-copying variant: proven, then shipped

Closing the independent-copy gap above means a snapshot mode that actually copies bundle bytes into the target repository instead of writing a pointer. That began as a proof rather than a feature and has since shipped, and the reasoning below has been overtaken in one specific way, so read it with that in mind.

**What was proven.** `DeepSnapshotOrchestrationIT` builds a `LazyBundleDirectory` over a published manifest with no live shard, engine, or local directory anywhere, wraps it in a `Store`, opens its `IndexCommit`, and hands both to core's existing `Repository#snapshotShard` — which takes the store and the commit *as parameters* rather than reading them off a shard. The bytes land in an `fs` repository, the snapshot is finalized, and core's ordinary `_restore` then restores it under a new name with every document back. It is a genuine independent copy in the standard repository format, restorable by a cluster that has never heard of this plugin, incremental for free because the repository dedupes by name, length, and checksum.

**Why the objection below no longer applies.** It was a valid objection to the *server-side-copy optimization*, not to the operation. If the node moves the bytes, server-side copy was never on the table, and the repack-versus-bulk-copy tension it creates disappears with it: Lucene-file granularity is what the repository format already wants, so the copy is correctly sized without repacking anything. No new `BlobContainer` SPI, no repack format, no new restore path, no temporary disk.

**Two things the doing taught.** `Store#getMetadata` asserts the commit's directory is identity-equal to the store's own, so the commit has to be listed from `store.directory()` rather than from the lazy directory underneath — a good assertion, and what stops a snapshot copying from one directory while describing another. And the copy distributes while the finalization does not: `finalizeSnapshot` submits a cluster state update, so it fails `NotClusterManagerException` anywhere else, while the copy itself ran happily on a data node. A deep snapshot is therefore one cluster-manager operation per snapshot with all the byte movement off it, which is the right shape for this branch.

**What was missing was the shipping, not the answer — and it has since been built.** `IndexDeepSnapshotAction`/`ShardDeepSnapshotAction` and their transports pin the generation, drive the copy per shard, finalize on the cluster manager, and release the pin on both paths, exposed at `POST /_plugins/_serverless/storage/index/{index}/_snapshot_deep/...`. The pin lifecycle it needed — which matters more here than for the pointer snapshot, because a deep snapshot's repository entry carries no pin id and so the repository cannot act as the ledger — is `PinLedger` plus `PinLedgerSweepTask`. See [Durability Posture](/design/durability-posture/).

The original objection follows, kept because its reasoning about the optimization still stands.
