...
### A real byte-copying variant: proven, not shipped

Closing the independent-copy gap above means a snapshot mode that actually copies bundle bytes into the target repository instead of writing a pointer. That now exists as a proof and not as a feature, and the reasoning below has been overtaken in one specific way, so read it with that in mind.
...
**What is missing is the shipping, not the answer**: a transport action that pins the generation, drives the copy per shard, finalizes on the cluster manager, and releases the pin on both paths; the deep-versus-shallow setting; and a pin lifecycle, which matters more here than for the pointer snapshot because a deep snapshot's repository entry carries no pin id, so the repository cannot act as the ledger. See [Durability Posture](/design/durability-posture/).
